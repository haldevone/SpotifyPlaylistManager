  ...
  return <>
            ...
            {showlist && dataComplete.map((item, i) => 
                        <PlayListCard key={item.id} background={i % 2 === 0 && "Snow"}>
                            ...
                            {<p className='playlist-tracks'>{item.tracks.total && item.tracks.total}</p>}
                            {<p className='playlist-followers'>{item.followers.total && item.followers.total.toLocaleString()}</p>}
                        </PlayListCard>
                    )}
  </>
